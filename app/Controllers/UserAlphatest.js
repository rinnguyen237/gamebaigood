									...
									try {
										...
										UserInfo.create({'id':client.UID, 'name':name, 'joinedOn':new Date(), 'ip':ip}, function(errC, user){
											if (!!errC) {
												...
												Message.create({'uid': client.UID, 'title':'Thành Viên Mới', 'text':'Chào mừng bạn đến với Soul68.CLUB, Hãy tham gia các sự kiện event Fanpage để nhận nhiều phần thưởng có giá trị', 'time':new Date()});
